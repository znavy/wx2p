...
class SendTextHandler(handler.base.BaseHandler):
    ...
    def get(self):
        ...
        if None in [user_str, content]:
            ret = dict(
                errCode=10001, errMsg='Missing parameter to_user/content')
            self.write(json.dumps(ret))
            self.finish()
            return

        users = user_str.split('|') if not isinstance(
            user_str, list) else user_str
...
class UserHandler(handler.base.BaseHandler):
    ...
    def post(self):
        ...
        data = dict(
            userid=userid, name=name, mobile=mobile, department=department)
