...
define('EncodingAESKey',
       default='RI3mMWIJnxgiSzTLV3VdVghArmqAyHtgSHK1sLxiwAC', help='')
...
class IndexHandler(handler.base.BaseHandler):
    ...
    def initialize(self):
        ...
        self.wxcpt = WXBizMsgCrypt(
            options.Token, options.EncodingAESKey, options.CorpID)
        self.deploy = Deploy()
    ...
    def msgHandle(self, xmlStr):
        ...
        if msgDict and msgDict.has_key('MsgType') and msgDict['MsgType'] == 'text':
            if msgDict.has_key('Content') and msgDict['Content'] == 'chunghwa56':
                ...
                text = None
            to_msg_dict = lib.util.buildMsgDict(
                text, msgDict['FromUserName'], 'text', msgDict['ToUserName'])
