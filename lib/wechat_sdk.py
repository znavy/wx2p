...
class WeChatEnterprise(object):
    ...
    def __post(self, url, data):
        resp = r.post(url, data=json.dumps(data).decode(
            'unicode-escape').encode("utf-8")).json()
    ...
    def send_msg2user(self, access_token, content, to_user=None, to_ptmt=None, to_tag=None, safe=0, msg_type="text", **kwargs):
        url = "%s/message/send?access_token=%s" % (
            self.url_prefix, access_token)
        data = {
            "safe": safe,
            "msgtype": msg_type,
    ...
    def get_department_list(self, access_token):
        url = "%s/department/list?access_token=%s" % (
            self.url_prefix, access_token)
        status, resp = self.__get(url)
        return status, resp

    def create_user(self, access_token, data):
        url = "%s/user/create?access_token=%s" % (
            self.url_prefix, access_token)
